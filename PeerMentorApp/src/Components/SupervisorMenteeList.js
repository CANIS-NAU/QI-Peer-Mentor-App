...
export default function SupervisorMenteeList( {navigation, mentor} ) {
  ...
  useEffect(() => {
    console.log(`supervisor mentee list${JSON.stringify(mentor)}`)
    getAsyncItem("current user").then(async user => {
      setCurrentUser(user)
      //console.log(`currentUser user_name: ${user.user_name}`)
      getMentees(mentor.user_id).then(result => {
        setMentees(result)
  ...
  const MenteeItem = ({mentee}) => (
    <View style={styles.homescreenmenteelist}>
        <Pressable style={styles.homescreenmentee}
                            onPress={() => navigation.navigate("Mentee Profile", {screenname: `${mentee.first_name} ${mentee.last_name}`, mentee: mentee, mentor: mentor})}>
